...
class Ui_MainWindow(QtWidgets.QMainWindow):
  ...
  def setupUi(self):
    ...
    # self.setWindowTitle("Kist Auto SAD Profile program (version 1.0)")
    self.setWindowTitle("Auto SAD Profile Program (ASAP) ")
    ...
    # text : original transmission point
  ...
  def click_dataload(self):
    ...
    for i in fnames[0]:
        ...
        if len(ext)>0 and ext=='.dm4':
            self.data_num += 1
            path_list.append(i)


    self.fnames = path_list
    self.horizontalSlider_2.setRange(1, self.data_num) #슬라이더 범위 설정
    ...
    self.plot_view_1()
    # 슬라이더 -> 이미지 연결
    self.horizontalSlider_2.sliderMoved.connect(self.plot_view_1)
  ...
  def click_start(self):
    ...
    # self.fnames = ['D:/kasad/data5/Dataset4_Hour_00_Minute_04_Second_04_Frame_0006.dm4', 'D:/kasad/data1/Dataset2_Hour_00_Minute_00_Second_06_Frame_0005.dm4']
    for f in self.fnames:
      k = KASAD(f)
      k.remove_beamstop()
      k.find_center(trials=trials)
      k.merge()
      cmap = plt.cm.gist_rainbow
      norm = plt.Normalize(0, 9)
      edges = k.edges;points = k.points;thresh_ransac = k.thresh_ransac
      from skimage.measure import ransac, CircleModel, label, regionprops

      for i,trials in enumerate([50]):
        model_robust, inliers = ransac(points, CircleModel, min_samples=5, residual_threshold=2, max_trials=trials)
        cy, cx, r = model_robust.params
        cir = plt.Circle((cx,cy), r, color=cmap(norm(i)), fill=False)
        center = np.array([cx + k.minx - k.distance_limit, cy + k.miny - k.distance_limit])
        (x_axis, radialprofile) = k.radial_profile(center = center)
  ...
  def plot_view_3(self):
    ...
    range = 600
    org_image = self.org_images[order][center_y-range : center_y+range,center_x-range : center_x+range]
  ...
  def plot_view_4(self):
    ...
    self.bar = pg.ColorBarItem( values= (org_image.min(), org_image.max()), colorMap=self.cm ) 
    self.bar.setImageItem(plot_widget)
